...
%[coord_sph,Tri,~,Ue]=ParticleSampleSphere('N',200);
...
%Optim = nav_opt(Grasp, handle , coord_sph, ext_Wrench);
...
function Grasp = rls_n (Grasp, handle, ext_Wrench)
  ...
  for j = 1:rows
      ...
      Grasp.weights(:,end+1)= sol.weights(:,1);
  end
  
end

function final_fit=fit_check(l,coord_sph,ext_Wrench)
 ...
 final_fit = coord_sph(temp); 
end

function Grasp = nav_opt(Grasp, handle,coord_sph, ext_Wrench)
 comb = nchoosek(handle,2);
 Grasp.weights=[];
 [rows,] = size(comb);
 ...
 for j = 1:rows
      c(1:3,:) = [Grasp.handForce(comb(j,1),:); Grasp.handForce(comb(j,2),:); Grasp.handForce(5,:) ];
      c=c';      
      %Grasp.weights(end+1,:) = fit_check(c,coord_sph,ext_Wrench);
  end
end
